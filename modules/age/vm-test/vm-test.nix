...
let
  ...
  secretsForTest = {
    ...
    portal-client-id = "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx";
    ...
    yinfeng-asciinema-token = "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx";
    yinfeng-id-ed25519 = "";
    yinfeng-nix-access-tokens = "";
  };
  rage = "${pkgs.rage}/bin/rage";
  buildSecret = name: content: ''
    "${rage}" --encrypt --recipient "${publicKey}" --output "$out/${name}.age" <<EOF
      ${content}
    EOF
  '';
