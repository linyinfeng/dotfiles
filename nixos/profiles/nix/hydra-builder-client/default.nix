...
in {
  ...
  environment.etc."${dir}/machines".text = ''
    ...
    hydra-builder@fsn0 aarch64-linux                         /etc/${keyFile} 2 50
  '';
  environment.etc."${dir}/machines-workstation".text = ''
    ...
    hydra-builder@fsn0      aarch64-linux /etc/${keyFile} 2 50 benchmark,big-parallel
    hydra-builder@nuc       aarch64-linux /etc/${keyFile} 8 100 kvm,nixos-test
    hydra-builder@xps8930   aarch64-linux /etc/${keyFile} 8 100 kvm,nixos-test
    hydra-builder@framework aarch64-linux /etc/${keyFile} 8 100 kvm,nixos-test
  '';
