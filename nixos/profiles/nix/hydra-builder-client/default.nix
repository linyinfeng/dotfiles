...
in
{
  ...
  environment.etc."${dir}/machines".text = ''
    ...
    hydra-builder@fsn0 aarch64-linux                         /etc/${keyFile} 2 200 benchmark,big-parallel
  '';
  environment.etc."${dir}/machines-workstation".text = ''
    ...
    hydra-builder@nuc       aarch64-linux /etc/${keyFile} 8 50
    hydra-builder@xps8930   aarch64-linux /etc/${keyFile} 8 50
    hydra-builder@framework aarch64-linux /etc/${keyFile} 8 50
  '';
