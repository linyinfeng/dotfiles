...
lib.mkMerge [
  # sicp-staging: OJ compose stack isolated in a dedicated user's rootless podman
  {
    ...
    # the main sops unit renders the staging config (manifest.json, not
    # manifest-for-users.json); it must run after the persistent home mount,
    ...
    systemd.tmpfiles.rules = [
      "Z ${composeDir} 0755 sicp-staging sicp-staging - -"
      "d ${composeDir} 0755 sicp-staging sicp-staging - -"
      "d ${composeDir}/config 0755 sicp-staging sicp-staging - -"
    ];
  ...
  # system podman (used by the gitlab-runner) extracts image blobs into
  # TMPDIR; on mtl0 the root tmpfs is only 2G, so pin it to persistent storage
  {
    environment.etc."containers/containers.conf.d/99-image-tmpdir.conf".text = ''
      [engine]
      image_copy_tmp_dir = "/var/lib/containers/tmp"
    '';
    # podman does not create the image_copy_tmp_dir itself
    systemd.tmpfiles.rules = [
      "d /var/lib/containers/tmp 1777 root root - -"
      "z /var/lib/containers/tmp 1777 root root - -"
    ];
  }
