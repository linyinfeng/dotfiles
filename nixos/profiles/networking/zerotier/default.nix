{
  ...
  localConfFile = json.generate "local.conf" cfg.localConf;
in {
  options = {
    services.zerotierone.localConf = lib.mkOption {
      type = lib.types.submodule {
        freeformType = json.type;
        options = {
          settings.interfacePrefixBlacklist = lib.mkOption {
  ...
  config = {
    ...
    systemd.services.zerotierone-presetup = {
      script = ''
        ...
        ln -sf "${localConfFile}" "${stateDir}/local.conf"
      '';
      serviceConfig = {
        Type = "oneshot";
